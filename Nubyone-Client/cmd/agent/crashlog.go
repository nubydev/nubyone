...
func crashLogDir() string {
        ...
        // Test write permission before using the exe directory.
        testFile := filepath.Join(dir, ".nubyone_write_test")
        f, werr := os.OpenFile(testFile, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0600)
        if werr != nil {
                return os.TempDir()
        }
        f.Close()
