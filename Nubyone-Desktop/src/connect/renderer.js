...
(async () => {
  try {
    const saved = await window.nubyone.getSavedConnection();
    if (saved) {
  ...
  try {
    const err = await window.nubyone.getPendingError();
...
form.addEventListener("submit", async (e) => {
  ...
  try {
    const result = await window.nubyone.connectToServer({ host, port, useTLS });

    if (result.success) {
      setStatus("Connected! Loading Nubyone…", "ok");
    } else {
